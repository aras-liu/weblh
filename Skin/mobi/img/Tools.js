...
$(function () {
    //搜索框   开始
    $("#ser-in").unbind().bind("click", searchKeyWord);
    //搜索框   
});


function reImg(obj,iw) {
obj.height=parseInt(iw/2);
alert(parseInt(iw/2))
}
...
function searchKeyWord() {
	...
	var type = $(this).find("#ClassID").val();
    var kwd = $(this).find("#se_key").val();
    if (kwd != "请输入关键词") {
        url = "case.asp?ClassID=" + type + "&kwd=" + kwd;
...
var url = "";
url = "case.asp?ClassID=" + selObj.options[selObj.selectedIndex].value;
        window.location.href = url;
...
function getContent(proId, currPage) {
    ...
    $.post("/Mobile/MAjax.ashx?action=GetContentByProductId&t=" + Math.random(), { ProId: proId, CurrPage: currPage }, function (rsp) {
        if ($(rsp).find("Error").length > 0) {
            ...
            $("#AppendContent").empty();
            $("#AppendContent").append($(rsp).find("Content").text());
