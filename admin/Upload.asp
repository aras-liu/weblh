...
<!--#Include File="../Class/Cls_Jpeg.asp"-->
<!--#Include File="../Class/JSON_2.0.4.asp"--><%

TempArr=Split(Fk_Site_Jpeg,"|-|Fangka|-|")
Jpeg_Pic=Clng(TempArr(0))
Jpeg_Pic_w=TempArr(1)
Jpeg_Pic_h=TempArr(2)
Jpeg_EditPic=Clng(TempArr(3))
Jpeg_EditPic_w=TempArr(4)
Jpeg_EditPic_h=TempArr(5)
...
Jpeg_WaterPicTransparence=TempArr(12)
Jpeg_WaterPicBgColor=TempArr(13)
Jpeg_WaterPosition=TempArr(14)
Jpeg_Water_x=TempArr(15)
Jpeg_Water_y=TempArr(16)
Jpeg_WaterFontSize=Clng(TempArr(17))
Set FKJpeg=New Cls_Jpeg
Dim hash

If Fk_Site_Edit=0 Or Instr("2345",Request.QueryString("immediate"))>0 Then
	Response.Write UploadFile("FileData")
ElseIf Fk_Site_Edit=1 Then
	Response.Write UploadFile("imgFile")
End If
...
function uploadfile(inputname)
	...
	attachdir=FileDir&"Up"'上传文件保存路径，结尾不要带/
	dirtype=1'1:按天存入目录 2:按月存入目录 3:按扩展名存目录  建议使用按天存
	maxattachsize=209715200'最大上传大小，默认是2M
	if immediate="1" or immediate="3" or immediate="5" then
		upext=Fk_Site_LinkExt&","&Fk_Site_PicExt&","&Fk_Site_FlashExt&","&Fk_Site_MediaExt
	elseif immediate="2" then
		upext=Fk_Site_PicExt
	elseif immediate="4" then
		upext=Fk_Site_LinkExt
	end if
	...
	if upfile.isErr then
		...
		if upfile.isErr then
			...
			msg=target
			Pic=target
			if immediate="1" And Fk_Site_Edit=0 then
				if Jpeg_EditPic=1 And Instr(",jpg,jpeg,jpe,bmp,gif,",","&LCase(extension)&",")>0 then
					SmallPic=Replace(Pic,".","_small.")
					SmallPic=FKJpeg.CreateSmall(Pic,SmallPic,Jpeg_EditPic_w,Jpeg_EditPic_h)
				else
					SmallPic=Pic
				end if
			elseif immediate="3" Then
				if Jpeg_Pic=1 And Instr(",jpg,jpeg,jpe,bmp,gif,",","&LCase(extension)&",")>0 then
					SmallPic=Replace(Pic,".","_small.")
					SmallPic=FKJpeg.CreateSmall(Pic,SmallPic,Jpeg_Pic_w,Jpeg_Pic_h)
				else
					SmallPic=Pic
				end if
			end if
			If immediate="1" Or immediate="3" Then
				if Jpeg_Water=1 And Instr(",jpg,jpeg,jpe,bmp,gif,",","&LCase(extension)&",")>0 then
					call FKJpeg.CreateWaterText(Pic,Jpeg_WaterText,Jpeg_WaterFontColor,Jpeg_WaterFont,Jpeg_WaterFontSize,Jpeg_WaterFontWeight,Jpeg_WaterPosition,Jpeg_Water_x,Jpeg_Water_y,1,Jpeg_WaterPic,Jpeg_WaterPicTransparence,Jpeg_WaterPicBgColor)
					if Fk_Site_Edit=0 Or immediate="3" then
						if Pic<>SmallPic then
							call FKJpeg.CreateWaterText(SmallPic,Jpeg_WaterText,Jpeg_WaterFontColor,Jpeg_WaterFont,Jpeg_WaterFontSize,Jpeg_WaterFontWeight,Jpeg_WaterPosition,Jpeg_Water_x,Jpeg_Water_y,1,Jpeg_WaterPic,Jpeg_WaterPicTransparence,Jpeg_WaterPicBgColor)
						end if
					end if
				end if
				if Jpeg_Water=2 And Instr(",jpg,jpeg,jpe,bmp,gif,",","&LCase(extension)&",")>0 then
					call FKJpeg.CreateWaterText(Pic,Jpeg_WaterText,Jpeg_WaterFontColor,Jpeg_WaterFont,Jpeg_WaterFontSize,Jpeg_WaterFontWeight,Jpeg_WaterPosition,Jpeg_Water_x,Jpeg_Water_y,2,Jpeg_WaterPic,Jpeg_WaterPicTransparence,Jpeg_WaterPicBgColor)
					if Fk_Site_Edit=0 Or immediate="3" then
						if Pic<>SmallPic then
							call FKJpeg.CreateWaterText(SmallPic,Jpeg_WaterText,Jpeg_WaterFontColor,Jpeg_WaterFont,Jpeg_WaterFontSize,Jpeg_WaterFontWeight,Jpeg_WaterPosition,Jpeg_Water_x,Jpeg_Water_y,2,Jpeg_WaterPic,Jpeg_WaterPicTransparence,Jpeg_WaterPicBgColor)
						end if
					end if
				end if
			End If
			If SiteDir<>FileDir Then
				Pic=Replace(Pic,FileDir,SiteDir,1)
				SmallPic=Replace(SmallPic,FileDir,SiteDir,1)
			End If
			if immediate="1" then
				if Fk_Site_Edit=0 then
					if Instr(","&Fk_Site_PicExt&",",","&LCase(extension)&",")>0 then
						msg="!"+SmallPic+"||"+Pic
					else
						msg="!"+Pic
					end if
	...
	elseif immediate="2" or immediate="4" or immediate="5" then
		if err<>"" Then
			uploadfile="<script language=""javascript"">alert('"&err&"');location.href='PicUpLoad.asp?Type="&immediate&"&Form="&Request.QueryString("Form")&"&Input="&Request.QueryString("Input")&"'</script>"
		else
			uploadfile="<style type=""text/css"">body,td,th {font-size:12px;}body {margin-left:3px;margin-top:3px;margin-right:0px;margin-bottom:0px;}a {font-size:12px;color:#000;}a:link {text-decoration:none;}a:visited {text-decoration:none;color:#000;}a:hover {text-decoration:none;color:#333;}a:active {text-decoration:none;color:#000;}</style>"
			uploadfile=uploadfile&"<script language=""javascript"">parent."&Request.QueryString("Form")&"."&Request.QueryString("Input")&".value='"&Pic&"';</script><a href='PicUpLoad.asp?Type="&immediate&"&Form="&Request.QueryString("Form")&"&Input="&Request.QueryString("Input")&"'>上传成功，如需重新上传请点这里！</a>"
		end if
	elseif immediate="3" then
		if err<>"" Then
			uploadfile="<script language=""javascript"">alert('"&err&"');location.href='PicUpLoad.asp?Type="&immediate&"&Form="&Request.QueryString("Form")&"&Input="&Request.QueryString("Input")&"'</script>"
			...
			uploadfile="<style type=""text/css"">body,td,th {font-size:12px;}body {margin-left:3px;margin-top:3px;margin-right:0px;margin-bottom:0px;}a {font-size:12px;color:#000;}a:link {text-decoration:none;}a:visited {text-decoration:none;color:#000;}a:hover {text-decoration:none;color:#333;}a:active {text-decoration:none;color:#000;}</style>"
			uploadfile=uploadfile&"<script language=""javascript"">parent.PicAdd('"&msg&"','"&Request.QueryString("Input")&"');location.href='PicUpLoad.asp?Type="&immediate&"&Form="&Request.QueryString("Form")&"&Input="&Request.QueryString("Input")&"';</script>"
		end if
	end if
end function
